...
public interface MatchApi {
	 ...
	 *         (status code 200)
	 */
	@Operation(operationId = "createMatch", summary = "Create a new match", tags = { "match" }, responses = {
			@ApiResponse(responseCode = "200", description = "A match, matchState and marketStates were successfully created.", content = {
					@Content(mediaType = "application/json", schema = @Schema(implementation = MatchDto.class))
			})
	})
	@RequestMapping(method = RequestMethod.POST, value = "/api/match", produces = { "application/json" }, consumes = {
			"application/json" })
	 ...
	 */
	@Operation(operationId = "getLatestMarketsByMatchId", summary = "Get the latest market states on match.", tags = {
			"markets" }, responses = {
					@ApiResponse(responseCode = "200", description = "the latest markets", content = {
							@Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = MarketStateDto.class)))
					})
			})
	@RequestMapping(method = RequestMethod.GET, value = "/api/match/{id}/markets/latest", produces = {
			"application/json" })
	@ResponseStatus(HttpStatus.OK)

	default List<MarketStateDto> getLatestMarketsByMatchId(
			@Parameter(name = "id", description = "id of the match", required = true, in = ParameterIn.PATH) @PathVariable("id") Long id)
			throws Exception {
		getRequest().ifPresent(request -> {
			for (MediaType mediaType : MediaType.parseMediaTypes(request.getHeader("Accept"))) {
				if (mediaType.isCompatibleWith(MediaType.valueOf("application/json"))) {
	 ...
	 */
	@Operation(operationId = "getMatchById", summary = "Get match on id.", tags = { "match" }, responses = {
			@ApiResponse(responseCode = "200", description = "the found match", content = {
					@Content(mediaType = "application/json", schema = @Schema(implementation = MatchDto.class))
			})
	})
	@RequestMapping(method = RequestMethod.GET, value = "/api/match/{id}", produces = { "application/json" })
	@ResponseStatus(HttpStatus.OK)

	default MatchDto getMatchById(
			@Parameter(name = "id", description = "Id of match to return.", required = true, in = ParameterIn.PATH) @PathVariable("id") Long id)
			throws Exception {
		getRequest().ifPresent(request -> {
			for (MediaType mediaType : MediaType.parseMediaTypes(request.getHeader("Accept"))) {
				if (mediaType.isCompatibleWith(MediaType.valueOf("application/json"))) {
	 ...
	 *         (status code 200)
	 */
	@Operation(operationId = "updateMatchAndStates", summary = "Update a live match with a new matchState", tags = {
			"match" }, responses = {
					@ApiResponse(responseCode = "200", description = "The match was updated and new matchstate and marketstates inserted.")
			})
	@RequestMapping(method = RequestMethod.PUT, value = "/api/match/{id}", consumes = { "application/json" })
	@ResponseStatus(HttpStatus.OK)

	default void updateMatchAndStates(
			@NotNull @Parameter(name = "X-XSRF-TOKEN", description = "", required = true, in = ParameterIn.HEADER) @RequestHeader(value = "X-XSRF-TOKEN", required = true) String X_XSRF_TOKEN,
			@Parameter(name = "id", description = "Id of match to update.", required = true, in = ParameterIn.PATH) @PathVariable("id") Long id,
			@Parameter(name = "MatchUpsertDto", description = "", required = true) @Valid @RequestBody MatchUpsertDto matchUpsertDto)
			throws Exception {
		throw new IllegalArgumentException("Not implemented");
